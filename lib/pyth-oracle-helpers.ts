...
export async function loadSponsoredOracle(
  ...
  const oracleAddress = findPythPushOracleAddress(feedId, PYTH_PUSH_ORACLE_ID, shardId);

  // 3) pull down the account
  const info = await connection.getAccountInfo(oracleAddress);
  if (!info) {
    throw new Error(`No Pyth-sponsored oracle found at ${oracleAddress.toBase58()}`);
  }
