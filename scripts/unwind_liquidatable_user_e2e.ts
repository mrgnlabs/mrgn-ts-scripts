...
async function main() {
  ...
  console.log("\n\n\n 4. WITHDRAW FROM NON-KAMINO BANKS BY LIQUIDATEE");
  let withdrawConfig = {
    PROGRAM_ID: config.PROGRAM_ID,
    ACCOUNT: state.liquidatee,
...
function parseConfig(rawConfig: string): Config {
  ...
  return {
    ...
    COLLATERAL_MINT: pkFromString(json.COLLATERAL_MINT),
    COLLATERAL_ORACLE: pkFromString(json.COLLATERAL_ORACLE),
...
export function parseState(raw: string): State {
  ...
  return {
    ...
    paddingBanks: json.paddingBanks.map(pkFromString),
  };
}
