...
async function main() {
  ...
  let driftWithdrawConfig = {
    PROGRAM_ID: config.PROGRAM_ID,
    BANK: state.driftBanks[6],
    ACCOUNT: state.liquidatee,
    AMOUNT: new BN(100), // doesn't matter, since we withdraw all
    WITHDRAW_ALL: false,
    ...
    KAMINO_ORACLE: config.KAMINO_RESERVE_ORACLE,
  };
  for (let i = 2; i < 8; i++) {
  ...
  remainingAccounts.push([state.debtBank, config.DEBT_ORACLE]);
  ...
  driftWithdrawConfig.NEW_REMAINING =
    composeRemainingAccounts(remainingAccounts);
  await withdrawDrift(true, driftWithdrawConfig, config.LIQUIDATEE_WALLET_PATH);
  await sleep(1000);
  return;
  //
  ...
  //   driftWithdrawConfig.NEW_REMAINING = composeRemainingAccounts(fullRemainingAccounts);
  //   await withdrawDrift(true, driftWithdrawConfig, config.LIQUIDATEE_WALLET_PATH);
  //   await sleep(1000);
  // }
